...
{	
	...
	if(Multi_ts_bitmap != -1){
		...
		render_bitmap(Multi_ts_bitmap, 0, 0, GR_RESIZE_MENU);
	}
	...
	for(idx=0;idx<MAX_WSS_SLOTS;idx++){
		...
		if(Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] == MULTI_TS_FLAG_EMPTY){
			...
			if((Netgame.type_flags & NG_TYPE_TEAM) && (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] != NULL) && (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx]->flags & NETINFO_FLAG_TEAM_CAPTAIN)){
				...
				render_string(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD] - 5,Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD] - 5, XSTR("C",737), GR_RESIZE_MENU);  // [[ Team captain ]]
			}
		}
	}	
}
...
void multi_ts_blit_wing_callsigns()
{
	...
	for(idx=0;idx<MAX_WSS_SLOTS;idx++){		
		...
		render_string(Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_X_COORD] + ((Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_W_COORD] - callsign_w)/2),Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_Y_COORD],callsign,GR_RESIZE_MENU);
	}	
}
	...
	for(idx = 0; idx < static_cast<int>(Ship_info.size()); idx++) {
		if(Ss_pool[idx] > 0){
			...
			if(ship_count < Multi_ts_avail_start){
				...
				render_string(Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_X_COORD] - 20,Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_Y_COORD],count,GR_RESIZE_MENU);
...
void multi_ts_blit_ship_info()
{
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Class",739), GR_RESIZE_MENU);
	if(strlen((sip->alt_name[0]) ? sip->alt_name : sip->name)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, temp, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Type",740), GR_RESIZE_MENU);
	if((sip->type_str != NULL) && strlen(sip->type_str)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->type_str, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Length",741), GR_RESIZE_MENU);
	if((sip->ship_length != NULL) && strlen(sip->ship_length)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->ship_length, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Max Velocity",742), GR_RESIZE_MENU);	
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start,str,GR_RESIZE_MENU);	
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Maneuverability",744), GR_RESIZE_MENU);
	if((sip->maneuverability_str != NULL) && strlen(sip->maneuverability_str)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->maneuverability_str, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Armor",745), GR_RESIZE_MENU);
	if((sip->armor_str != NULL) && strlen(sip->armor_str)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->armor_str, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Gun Mounts",746), GR_RESIZE_MENU);
	if((sip->gun_mounts != NULL) && strlen(sip->gun_mounts)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->gun_mounts, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Missile Banks",747), GR_RESIZE_MENU);
	if((sip->missile_banks != NULL) && strlen(sip->missile_banks)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->missile_banks, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Manufacturer",748), GR_RESIZE_MENU);
	if((sip->manufacturer_str != NULL) && strlen(sip->manufacturer_str)){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->manufacturer_str, GR_RESIZE_MENU);
	}
	y_start += line_height;
	...
	for(int idx=0;idx<Multi_ts_ship_info_line_count;idx++){
		...
		render_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, Multi_ts_ship_info_lines[idx], GR_RESIZE_MENU);
		y_start += line_height;
	}
...
void multi_ts_blit_status_bar()
{
	...
	if(blit){	
		...
		render_string(Multi_ts_status_coords[gr_screen.res][MULTI_TS_X_COORD] + ((Multi_ts_status_coords[gr_screen.res][MULTI_TS_W_COORD] - text_w)/2),Multi_ts_status_coords[gr_screen.res][MULTI_TS_Y_COORD],text,GR_RESIZE_MENU);
	}
}
...
void multi_ts_blit_carried_icon()
{
	...
	case MULTI_TS_PLAYER_LIST:
		...
		render_string(x + offset_x,y + offset_y,Multi_ts_team[Net_player->p_info.team].multi_ts_player[Multi_ts_carried_from_index]->m_player->callsign,GR_RESIZE_MENU);
		break;
	default : 
		break;			
	}
