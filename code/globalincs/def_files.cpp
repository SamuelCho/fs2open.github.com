...
"vertIn vec3 vertNormal;\n"
"vertIn vec4 vertTangent;\n"
"vertIn float vertModelID;\n"
...
"vertOut vec3 fragEnvReflect;\n"
"#endif\n"
"#ifdef FLAG_NORMAL_MAP\n"
"vertOut mat3 fragTangentMatrix;\n"
"#endif\n"
"#ifdef FLAG_FOG\n"
"uniform float fogStart;\n"
"uniform float fogScale;\n"
"vertOut float fragFogDist;\n"
"#endif\n"
...
"vertOut float geoNotVisible;\n"
"#else\n"
"vertOut float fragNotVisible;\n"
"#endif\n"
"#endif\n"
"#ifdef FLAG_SHADOW_MAP\n"
"vertOut float geoInstance;\n"
"uniform mat4 shadow_proj_matrix[4];\n"
"#endif\n"
"#ifdef FLAG_SHADOWS\n"
"uniform mat4 shadow_mv_matrix;\n"
"uniform mat4 shadow_proj_matrix[4];\n"
"vertOut vec4 fragShadowUV[4];\n"
"vertOut vec4 fragShadowPos;\n"
"#endif\n"
"#ifdef FLAG_THRUSTER\n"
...
"#ifdef FLAG_SHADOW_MAP\n"
"vertOut float geoClipDistance;\n"
"#else\n"
"vertOut float fragClipDistance;\n"
"#endif\n"
"#endif\n"
...
"	}\n"
" #endif\n"
"	fragPosition = modelViewMatrix * orient * vertex;\n"
" #ifdef FLAG_SHADOW_MAP\n"
...
"	fragEnvReflect = reflect(normalize(fragPosition.xyz), normal);\n"
"	fragEnvReflect = vec3(envMatrix * vec4(fragEnvReflect, 0.0));\n"
"	fragEnvReflect = normalize(fragEnvReflect);\n"
" #endif\n"
" #ifdef FLAG_FOG\n"
...
"fragIn vec3 fragEnvReflect;\n"
"#endif\n"
"#ifdef FLAG_NORMAL_MAP\n"
"uniform sampler2D sNormalmap;\n"
"fragIn mat3 fragTangentMatrix;\n"
"#endif\n"
"#ifdef FLAG_FOG\n"
"uniform vec4 fogColor;\n"
"fragIn float fragFogDist;\n"
"#endif\n"
"#ifdef FLAG_ANIMATED\n"
...
"#endif\n"
"#ifdef FLAG_SHADOWS\n"
"fragIn vec4 fragShadowUV[4];\n"
"fragIn vec4 fragShadowPos;\n"
...
"#ifdef FLAG_CLIP\n"
"uniform int use_clip_plane;\n"
"fragIn float fragClipDistance;\n"
"#endif\n"
"fragIn vec4 fragPosition;\n"
"fragIn vec3 fragNormal;\n"
...
"{\n"
"	return tex2DArray(shadow_map, vec3(uv + offset_uv * shadowMapSizeInv, float(cascade))).xy;\n"
"}\n"
"\n"
...
"   vec2 normalSample;\n"
"   normal.rg = normalSample = (tex2D(sNormalmap, texCoord).ag * 2.0) - 1.0;\n"
...
"   }\n"
" #endif\n"
"	baseColor = tex2D(sBasemap, diffuseTexCoord);\n"
...
"#ifdef FLAG_SPEC_MAP\n"
"   vec4 specColour = tex2D(sSpecmap, texCoord);\n"
...
"	teamMask = tex2D(sMiscmap, texCoord);\n"
...
"          dist = distance(lightPosition[i].xyz, fragPosition.xyz);\n"
...
"      // Ambient and Diffuse\n"
"      lightDiffuse += (lightDiffuseColor[i].rgb * diffuseFactor * (max(dot(normal, lightDir), 0.0)) * attenuation) * shadow;\n"
"      // Specular\n"
"      float NdotHV = clamp(dot(normal, half_vec), 0.0, 1.0);\n"
"      lightSpecular.rgb += ((lightSpecColor[i] * pow(NdotHV, specPower)) * attenuation) * specularIntensity * shadow;\n"
"   }\n"
"	lightAmbientDiffuse = lightAmbient + lightDiffuse;\n"
"#else\n"
"   lightAmbientDiffuse = color.rgb;\n"
"#endif\n"
"\n"
...
" #ifdef FLAG_LIGHT\n"
"      baseColor.rgb = mix(tex2D(sFramebuffer, screenPos + distort*anim_timer + anim_timer*0.1*normal.xy).rgb,baseColor.rgb,clamp(cloak_interp+anim_timer*2.0,0.0,1.0));\n"
" #else\n"
"      baseColor.rgb = mix(tex2D(sFramebuffer, screenPos + distort*anim_timer + anim_timer*0.1*fragNormal.xy).rgb,baseColor.rgb,clamp(cloak_interp+anim_timer*2.0,0.0,1.0));\n"
" #endif\n"
"   }\n"
"#endif\n"
"#ifdef FLAG_CLIP\n"
...
"uniform mat4 shadow_proj_matrix[4];\n"
"varying in float geoInstance[3];\n"
"varying out vec4 fragPosition;\n"
"#ifdef FLAG_TRANSFORM\n"
"varying in float geoNotVisible[];\n"
"varying out float fragNotVisible;\n"
"#endif\n"
"#ifdef FLAG_CLIP\n"
"varying in float geoClipDistance[];\n"
"varying out float fragClipDistance;\n"
"#endif\n"
"void main(void)\n"
"{\n"
...
"			gl_Position.z = -1.0;\n"
"       fragPosition = gl_PositionIn[vert];\n"
"		gl_Layer = instanceID;\n"
"#ifdef FLAG_TRANSFORM\n"
...
"	vec4 sum = tex2D(tex, fragTexCoord.xy) * BlurWeights[0];\n"
"#ifdef PASS_0\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += tex2D(tex, vec2(clamp(fragTexCoord.x - float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0), fragTexCoord.y)) * BlurWeights[i];\n"
"		sum += tex2D(tex, vec2(clamp(fragTexCoord.x + float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0), fragTexCoord.y)) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"#ifdef PASS_1\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += tex2D(tex, vec2(fragTexCoord.x, clamp(fragTexCoord.y - float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0))) * BlurWeights[i];\n"
"		sum += tex2D(tex, vec2(fragTexCoord.x, clamp(fragTexCoord.y + float(i) * (blurSize/BLUR_SIZE_DIV), 0.0, 1.0))) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"	fragOut0 = sum;\n"
...
"		color_in = tex2D(tex, fragTexCoord.xy + distort);\n"
"		vec4 color_bloom = tex2D(bloomed, fragTexCoord.xy + distort);\n"
...
"uniform sampler2D tex;\n"
"void main() {\n"
"	vec4 color = tex2D(tex, fragTexCoord.xy);\n"
...
"	 position_p = modelViewMatrix * vertPosition;\n"
"	 fragTexCoord = vertTexCoord;\n"
"	 fragColor = vertColor;\n"
"	#endif\n"
"	#ifdef FLAG_EFFECT_DISTORTION\n"
...
"	vec2 offset = vec2(radius_p * abs(0.5 - fragTexCoord.x) * 2.0, radius_p * abs(0.5 - fragTexCoord.y) * 2.0);\n"
...
"	vec4 sceneDepth = tex2D(depthMap, depthCoord);\n"
...
"	vec2 depthCoord = vec2(gl_FragCoord.x / window_width, gl_FragCoord.y / window_height);\n"
"	vec4 fragmentColor = tex2D(baseMap, fragTexCoord.xy) * fragColor.a;\n"
"	vec2 distortion = tex2D(distMap, fragTexCoord.xy+vec2(0.0, offset_out)).rg;\n"
"	float alpha = clamp(dot(fragmentColor.rgb,vec3(0.3333))*10.0,0.0,1.0);\n"
"	distortion = ((distortion - 0.5) * 0.01) * alpha;\n"
"	fragOut0 = tex2D(frameBuffer,depthCoord+distortion);\n"
"	fragOut0.a = alpha;\n"
"}";
...
"varying in vec4 geoColor[];\n"
"varying out float radius_p;\n"
"varying out vec4 position_p;\n"
"varying out vec4 fragTexCoord;\n"
...
"	position_p = pos;\n"
"	fragTexCoord = vec4(0.0, 0.0, 0.0, 0.0);\n"
"	radius_p = radius_g[0];\n"
...
"	gl_Position = projMatrix * pos;\n"
"	position_p = pos;\n"
"	fragTexCoord = vec4(0.0, 1.0, 0.0, 0.0);\n"
"	radius_p = radius_g[0];\n"
"	fragColor = geoColor[0];\n"
"	EmitVertex();\n"
...
"	gl_Position = projMatrix * pos;\n"
"	position_p = pos;\n"
"	fragTexCoord = vec4(1.0, 0.0, 0.0, 0.0);\n"
"	radius_p = radius_g[0];\n"
"	fragColor = geoColor[0];\n"
"	EmitVertex();\n"
...
"	gl_Position = projMatrix * pos;\n"
"	position_p = pos;\n"
"	fragTexCoord = vec4(1.0, 1.0, 0.0, 0.0);\n"
"	radius_p = radius_g[0];\n"
"	fragColor = geoColor[0];\n"
"	EmitVertex();\n"
"	EndPrimitive();\n"
...
"	fragOut0 = vec4(tex2DArray(shadow_map, texcoord).rgb,1.0);\n"
"	//fragOut0 = vec4(tex2D(shadow_map, fragTexCoord.xy).rgb,1.0);\n"
"}\n"; 
...
"	float decay = 1.0;\n"
"	vec4 sum = vec4(0.0);\n"
"	vec4 mask = tex2D(cockpit, fragTexCoord.st);\n"
"	if (mask.r < 1.0) {\n"
...
"		pos.st -= step;\n"
"		vec4 tex_sample = tex2D(scene, pos);\n"
"		if (tex_sample.r == 1.0)\n"
"			sum += decay * weight;\n"
"		decay *= falloff;\n"
"	}\n"
...
"void main()\n"
"{\n"
"	vec3 val = vec3(tex2D(ytex, fragTexCoord.st).r - 0.0625, tex2D(utex, fragTexCoord.st).r - 0.5, tex2D(vtex, fragTexCoord.st).r - 0.5);\n"
...
"	vec3 position = tex2D(PositionBuffer, screenPos).xyz;\n"
"\n"
...
"		discard;\n"
"	vec3 color = tex2D(ColorBuffer, screenPos).rgb;\n"
"	vec4 normal = tex2D(NormalBuffer, screenPos);\n"
"	vec3 specfactor = tex2D(SpecBuffer, screenPos).rgb;\n"
