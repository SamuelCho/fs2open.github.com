...
"	vec3 lightAmbient = (emissionFactor + ambientFactor) * aoFactor;\n"
...
"	vec4 sum = tex2DLod(tex, fragTexCoord.xy, float(level)) * BlurWeights[0];\n"
"#ifdef PASS_0\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += tex2DLod(tex, vec2(clamp(fragTexCoord.x - float(i) * (texSize) * tapSize, 0.0, 1.0), fragTexCoord.y), float(level)) * BlurWeights[i];\n"
"		sum += tex2DLod(tex, vec2(clamp(fragTexCoord.x + float(i) * (texSize) * tapSize, 0.0, 1.0), fragTexCoord.y), float(level)) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"#ifdef PASS_1\n"
"	for (int i = 1; i < 6; i++) {\n"
"		sum += tex2DLod(tex, vec2(fragTexCoord.x, clamp(fragTexCoord.y - float(i) * (texSize) * tapSize, 0.0, 1.0)), float(level)) * BlurWeights[i];\n"
"		sum += tex2DLod(tex, vec2(fragTexCoord.x, clamp(fragTexCoord.y + float(i) * (texSize) * tapSize, 0.0, 1.0)), float(level)) * BlurWeights[i];\n"
"	}\n"
"#endif\n"
"	fragOut0 = sum;\n"
...
"			factor += scale;\n"
"			bloom += tex2DLod(bloomed, fragTexCoord.xy, float(mipmap)).rgb * scale;\n"
"		}\n"
"		bloom /= factor;\n"
...
" #endif\n"
" // Global constant\n"
"	vec4 color_in = tex2D(tex, fragTexCoord.xy + distort);\n"
"	vec4 color_out;\n"
" // Bloom\n"
"	if (bloom_intensity > 0.0) {\n"
"		vec3 bloom = vec3(0.0, 0.0, 0.0);\n"
...
"			float scale = 1.0/exp2(float(mipmap));\n"
"			factor += scale;\n"
"			bloom += tex2DLod(bloomed, fragTexCoord.xy + distort, float(mipmap)).rgb * scale;\n"
"		}\n"
"		bloom /= factor;\n"
"		color_in.rgb += bloom;\n"
"	}\n"
...
"	vec4 specColor = tex2D(SpecBuffer, screenPos);\n"
