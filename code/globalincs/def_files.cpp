...
"      // Ambient and Diffuse\n"
"      lightDiffuse += (lightColor[i].rgb * light_factor * (max(dot(normal, lightDir), 0.0)) * attenuation) * shadow;\n"
"      // Specular\n"
"      float NdotHV = clamp(dot(normal, half_vec), 0.0, 1.0);\n"
"      lightSpecular.rgb += ((lightColor[i] * pow(NdotHV, gl_FrontMaterial.shininess)) * attenuation) * specularIntensity * shadow;\n"
"   }\n"
